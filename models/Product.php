...
class Product {
    ...
    public static function search() {
        ...
        if (!empty($_POST['nameStartsWith'])) { //Принимаем данные
            $search = trim(strip_tags(stripcslashes(htmlspecialchars($_POST['nameStartsWith']))));

            $db_search = $db->query("SELECT * FROM product WHERE name LIKE '%$search%'");

            $response = array();
            $i = 0;
            while ($row = $db_search->fetch()) {
                //echo "\n<li>" . $row['name'] . "</li>"; //$row["name"] - имя поля таблицы
                //$return_arr[] = $row['name'];
                //echo json_encode($return_arr);
                $response[] = array(
                    'name' => $row['name'],
                    'id' => $row['id'] // тут у меня функция формирует ссылку
                );
                $i++;
            }
        }
